...
- If a CDP action reports a stale or missing `d<N>` id, that id did not fire an action. Rediscover the target with `cdp_find_elements`, `cdp_take_dom_snapshot`, or `cdp_evaluate_script`; do not retry the same stale uid.
