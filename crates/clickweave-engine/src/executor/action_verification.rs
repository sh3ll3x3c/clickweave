...
///
/// Returns `Some((method, assertion))` only when both halves are set — a
/// thin owned wrapper around
/// [`NodeType::verification`](clickweave_core::NodeType::verification) that
/// resolves the optional method/assertion pair inside the substruct.
pub(crate) fn extract_verification_config(
    node_type: &NodeType,
) -> Option<(VerificationMethod, String)> {
    let cfg = node_type.verification()?;
    Some((
        cfg.verification_method?,
        cfg.verification_assertion.clone()?,
    ))
}
