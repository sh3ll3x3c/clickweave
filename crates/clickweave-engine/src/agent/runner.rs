...
pub struct StateRunner {
    ...
    pub(crate) completed_subgoal_extraction_queue: Vec<(usize, Milestone)>,
    /// Workflow node ids emitted via `AgentEvent::NodeAdded` since the
    /// most recent `push_subgoal`. Drained at `complete_subgoal` so the
    /// extractor records the new skill's `produced_node_ids` lineage.
    pub(crate) produced_node_ids_for_this_subgoal: Vec<uuid::Uuid>,
...
impl StateRunner {
    ...
    pub fn new_with_episodic_and_skills(
        ...
        Self {
            ...
            produced_node_ids_for_this_subgoal: Vec::new(),
        ...
        for m in muts {
            match m {
                TaskStateMutation::PushSubgoal { text } => {
                    ...
                    // Reset the per-subgoal node lineage at the
                    // matching push so the next CompleteSubgoal sees
                    // only the nodes produced inside this frame.
                    self.produced_node_ids_for_this_subgoal.clear();
                }
                TaskStateMutation::CompleteSubgoal { summary } => {
                    ...
                    {
                        Ok(milestone) => {
                            self.completed_subgoal_extraction_queue
                                .push((push_idx, milestone));
                        }
    ...
    async fn write_subgoal_completed_records(&mut self, count: usize, turn: &AgentTurn) {
        ...
        // its push-side `recorded_steps` index and the milestone payload
        // available without re-walking `task_state.milestones`.
        ...
        if !queue.is_empty() && self.skill_ctx.enabled && self.config.skills_enabled {
            ...
            // Snapshot the produced-node lineage once; the action sketch
            // for each subgoal frame consumes the same set (the
            // accumulator was reset on the matching push_subgoal).
            let produced_node_ids = std::mem::take(&mut self.produced_node_ids_for_this_subgoal);

            for (push_idx, milestone) in queue {
                let action_sequence = if push_idx < self.recorded_steps.len() {
                    self.recorded_steps[push_idx..].to_vec()
                } else {
                    Vec::new()
                };
                let pre_state_sig = crate::agent::skills::signature::compute_subgoal_signature(
                    &milestone.text,
                    &self.world_model,
                );
            ...
            match outcome {
                TurnOutcome::ToolSuccess {
                    ...
                    let produced_node_id = self
                        ...
                        .await;
                    // Spec 3: track every node produced inside the
...
mod skills_apply_mutations_tests {
    ...
    #[test]
    fn push_subgoal_resets_produced_node_lineage() {
        let mut r = StateRunner::new_for_test("g".to_string());
        r.produced_node_ids_for_this_subgoal
            .push(uuid::Uuid::new_v4());
        r.apply_mutations(&[TaskStateMutation::PushSubgoal {
            text: "open chat".into(),
        }]);
        assert!(r.produced_node_ids_for_this_subgoal.is_empty());
    }
