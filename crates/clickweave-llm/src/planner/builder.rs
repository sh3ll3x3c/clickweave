...
use super::parse::{id_str_short, layout_nodes, step_rejected_reason, truncate_intent};
use super::types::{
    PatchNodeUpdate, PatchResult, PatcherOutput, PlanEdge, PlanNode, PlanResult, PlanStep,
    PlannerGraphOutput,
};
...
pub(crate) fn build_patch_from_output(
    ...
    if !output.add.is_empty() && !output.add_nodes.is_empty() {
        ...
        let (add_steps, add_warnings) = parse_lenient::<super::plan::FlatPlanStep>(&output.add);
...
pub(crate) fn build_plan_as_patch(
    ...
    let (flat_steps, step_warnings) = parse_lenient::<super::plan::FlatPlanStep>(raw_steps);
...
pub(crate) fn build_graph_plan_as_patch(
    ...
    let mut next_id_counters = HashMap::new();
    let positions = layout_nodes(graph.nodes.len());
