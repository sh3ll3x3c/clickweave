...
### Agent Loop — Observe, Act, Persist

Describe your automation goal in natural language and Clickweave runs an **observe-act agent loop** against the target app. Each step:
- Observes the current app state (page elements via CDP when available; native accessibility otherwise)
- Asks the configured agent LLM for exactly one tool call, given the goal, the MCP tool list, and the prior step history
- Optionally gates the tool call behind user approval (pre-approve tool categories or approve step-by-step)
- Dispatches the tool via MCP and appends the call to the workflow as a new node — so authoring and execution happen on the same surface
- Runs context compaction every step, including snapshot supersession that collapses older AX/DOM snapshots to placeholders so only the most recent snapshot per tool stays at full fidelity

The agent stops when it emits `agent_done`, asks to `agent_replan`, or hits max-steps / max-consecutive-errors. The saved workflow is a linear sequence of tool calls that can be replayed deterministically afterwards.
...
Clickweave has two execution modes: **Test** and **Run**. In Test mode, the engine verifies each step after execution — a **fast VLM** describes the screen, then a **supervisor LLM** judges whether the step succeeded. Steps that pass continue automatically; on failure the UI presents **Retry**, **Skip**, or **Abort** options. Read-only nodes (FindText, FindImage, TakeScreenshot) are exempt from verification since they have no observable side-effect. LLM decisions made during Test (element resolution, click disambiguation, app-name resolution) are saved to a **decision cache**. In Run mode, the cache replays those decisions deterministically — skipping redundant LLM calls and running without supervision for faster, unattended execution.

### Inline Verification

Any read-only Tool step (`FindText`, `FindImage`, `TakeScreenshot`) can be marked with a Verification role, turning it into an inline test assertion that runs during execution:
...
- **Agent** — drives the observe-act agent loop; one tool call per step
- **Supervisor** — the LLM judge used in Test-mode step verification and as the VLM fallback for walkthrough enrichment (currently stored under the `plannerConfig` settings key, being renamed)
- **Fast VLM** (optional) — dedicated vision model for fast screenshot description before the supervisor runs its judge pass
...
1. **Describe, build, or record** — Type your automation goal for the agent loop, add nodes from the palette by hand, or record a walkthrough of the task
2. **Watch the agent** — The agent observes the app, asks the LLM for one tool call per step, and appends each emitted call to the workflow live. Approve individual actions or pre-approve tool categories.
3. **Review the workflow** — Inspect the materialized tool-call sequence, tweak node parameters, add verification nodes
4. **Test** — Re-run in **Test mode** with per-step supervision: after each step, a fast VLM + supervisor LLM verify the screen and pause on failures so you can Retry, Skip, or Abort. LLM decisions (element resolution, click disambiguation) are recorded to a decision cache.
5. **Run** — Switch to **Run mode** for unattended execution. The decision cache replays recorded choices deterministically — no LLM calls for previously resolved decisions, no supervision pauses.
...
│   ├── clickweave-engine/  # Agent loop + deterministic executor (retries, verification, supervision)
│   ├── clickweave-llm/     # LLM client, walkthrough enrichment, prompt templates
│   └── clickweave-mcp/     # MCP JSON-RPC client (subprocess lifecycle, tool calls)
├── src-tauri/              # Tauri app shell & IPC commands (agent, executor, walkthrough, project, runs)
├── ui/                     # React frontend (Vite + Tailwind CSS v4)
│   ├── src/
│   │   ├── components/     # Graph canvas, nodes, modals, agent cockpit, verdict bar
│   │   ├── store/          # Zustand state (project, execution, agent, history, settings, log, verdict, walkthrough, UI)
│   │   └── hooks/          # React hooks (undo/redo, node/edge sync, workflow actions, executor/agent events)
...
Saved workflows are linear sequences of these nodes; the agent loop emits one per step as it runs.
...
Toggle in **Settings**:
- **Fast VLM enabled** (default: off) — routes screenshot description to a dedicated fast VLM endpoint before the supervisor judge runs; when disabled, the supervisor works from the action trace alone
- **Tool permissions** — per-tool `ask` / `allow` policy for the agent approval gate
...
No. You can build workflows visually by adding nodes from a categorized palette, describe a goal in natural language and let the agent loop execute it step by step (materializing the workflow live), record a walkthrough of the task to generate a workflow automatically, or combine all three approaches.
...
Any provider with an OpenAI-compatible `/v1/chat/completions` endpoint: LM Studio, vLLM, Ollama (OpenAI-compatible mode), OpenRouter, OpenAI, and others. You can use different models for the agent loop, supervision, and fast vision tasks.
...
Traditional RPA tools rely on selectors, DOM scraping, or accessibility APIs tied to specific applications. Clickweave uses visual perception (screenshots + OCR + template matching) and can automate any application with a visible UI. The agent loop means you can describe what you want rather than manually scripting every click.

### How does workflow verification work?
Clickweave has two layers of verification. **Per-step supervision** (Test mode only) checks each step as it runs: a fast VLM describes the screen and a supervisor LLM decides pass/fail, pausing for user action on failure. **Inline verification** uses Verification-role nodes (any read-only tool step: FindText, FindImage, TakeScreenshot) as test assertions that produce pass/fail/warn verdicts during execution and fail-fast on failure.
...
- **Tauri Commands:** `src-tauri/src/commands/` — IPC bridge (`run_agent`, `stop_agent`, `approve_agent_action`, `run_workflow`, `stop_workflow`, `supervision_respond`)
