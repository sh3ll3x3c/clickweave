...
`host::run::run_agent(AgentRunParams)` (`crates/clickweave-host/src/run.rs`) is the engine-call seam used by all consumers. It dispatches to `run_agent_workflow` or `run_agent_workflow_with_prompt_override` (`crates/clickweave-engine/src/agent/mod.rs`) based on `system_prompt_override`. Both build a `StateRunner` and an `AgentTraceGraph` and drive the loop.
