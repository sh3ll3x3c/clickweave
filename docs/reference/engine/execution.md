...
- `agent://episodes_retrieved` — Spec 2 D33. Emitted by `StateRunner` when an episodic-retrieval pass returns at least one candidate. Triggered on run-start (step 0) and on the `Exploring/Executing -> Recovering` phase transition. Payload: `{ run_id, trigger: "run_start" | "recovering_entry", count, episode_ids: string[], scope_breakdown_workflow, scope_breakdown_global }`.
- `agent://episode_written` — Spec 2 D33. Emitted by the background `EpisodicWriter` task after a recovery snapshot is persisted to the workflow-local SQLite store. Payload: `{ run_id, episode_id, outcome: "inserted" | "merged" | "dropped: <reason>", occurrence_count }`.
- `agent://episode_promoted` — Spec 2 D33. Emitted by the run-terminal promotion pass after the writer copies one or more workflow-local episodes into the global cross-workflow store. Payload: `{ run_id, count, skipped }` (`skipped` is the number of candidates the `should_promote` gate rejected).
